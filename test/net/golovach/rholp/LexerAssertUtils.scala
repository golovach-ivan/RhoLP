...
import net.golovach.rholp.log.Diagnostic.Kind._
import net.golovach.rholp.log.{Diagnostic, DiagnosticListener}
...
object LexerAssertUtils extends Matchers {
  ...
  case class EqTo(diagnostics: util.List[Diagnostic]) {
    def eqTo(builders: DiagnosticBuilder*): Unit = {
      ...
      diagnostics should have size builders.length
